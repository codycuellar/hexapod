...
def nlerp(p1:Point, p2:Point, t:List[float], planar:bool=False):
    if planar:
...
def nquad_bez(p1:Point, p2:Point, p3:Point, t:List[float], planar:bool=False):
    if planar:
...
def cubic_bez(v1:float, v2:float, v3:float, v4:float, t:float):
...
def ncubic_bez(p1:Point, p2:Point, p3:Point, p4:Point, t:List[float], planar:bool=False):
    if planar:
...
def ncosine_ease(t:List[float], planar:bool=False):
    if planar:
        last = t[-1]
